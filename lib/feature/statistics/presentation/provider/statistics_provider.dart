...
@riverpod
Future<List<StatisticsParams>> statistics(StatisticsRef ref) async {
  ...
  Map<int, int> reportesPorMes = {};
  int contador = 0;
  if (listReport.isNotEmpty) {
    listReport.forEach((report) {
      int mes = DateAndHoursUtils.getMonth(report.dia);
      contador = DateAndHoursUtils.calcularDiferenciaHoras(
          report.horaInicio, report.horaFinal);
      reportesPorMes[mes] = reportesPorMes[mes] != null
          ? reportesPorMes[mes]! + contador
          : contador;
    });

    List<StatisticsParams> countHourByMonth =
        reportesPorMes.entries.map((entry) {
      return StatisticsParams(month: entry.key, countData: entry.value);
    }).toList();

    return countHourByMonth;
  }
